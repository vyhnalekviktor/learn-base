...
// ====== POUŽIJ CACHED WALLET S ČEKANÍM NA COMMON.JS ======

let cachedWallet = null;

try {

// Počkej na wallet cache z common.js (max 3s)

const cache = await window.BaseCampTheme.waitForWallet();

cachedWallet = cache.wallet;

console.log('✅ Wallet cache ready:', cachedWallet, 'Sepolia:', cache.sepolia_status);

} catch (err) {

// Timeout - zkus fallback na přímý localStorage

console.log('⏱️ Wallet cache timeout, trying localStorage fallback:', err);

cachedWallet = localStorage.getItem('cached_wallet');
