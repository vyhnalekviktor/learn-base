...
def call_nft_api(wallet_address, approve_first=True):
    thirdweb_api_key = os.getenv("THIRDWEB_API_KEY")
    if not thirdweb_api_key:
        return {"error": "Missing THIRDWEB_API_KEY env variable"}
    ...
    NFT_CONTRACT = os.getenv("BATCH1_NFT_ADDRESS", "0xA76F456f6FbaB161069fc891c528Eb56672D3e69")
    USDC_AMOUNT = "4000000"  # 4 USDC (6 decimals)
    ...
    try:
        if approve_first:
            ...
            approve_payload = {
                "calls": [{
                    ...
                    "params": [NFT_CONTRACT, USDC_AMOUNT]
                }],
                "chainId": 8453,
                "from": wallet_address
            }

            approve_resp = requests.post(url, headers=headers, json=approve_payload)
            approve_data = approve_resp.json()
            print(f"Approve result: {approve_data}")
            results["approve"] = approve_data

            time.sleep(3)  # Čekej na potvrzení

        # 2. PLNÁ CLAIM SIGNATURA z tutorialu
        print("2. Claiming NFT...")
        claim_payload = {
            "calls": [{
                ...
                "params": [
                    wallet_address,  # _receiver
                    0,  # _tokenId
                    1,  # _quantity
                    USDC_ADDRESS,  # _currency
                    USDC_AMOUNT,  # _pricePerToken
                    {  # _allowlistProof
                        "proof": [],
                        "quantityLimitPerWallet": 1,
                        "pricePerToken": USDC_AMOUNT,
        ...
        claim_resp = requests.post(url, headers=headers, json=claim_payload)
        claim_data = claim_resp.json()
        print(f"Claim result: {claim_data}")

        results["claim"] = claim_data
        return results
...
async def mint_nft(request: Request):
    ...
    response = call_nft_api(wallet)

    if "error" in response:
        raise HTTPException(status_code=400, detail=response["error"])

    # TYPE SAFE - použij .get() a explicitní typing
    claim_data: dict = response.get("claim", {})
    tx_hash = claim_data.get("transactionHash")

    if tx_hash:
        # DB update (volitelné)
        try:
            database.update_field("USER_INFO", "nft_minted", wallet, True)
        except:
            pass

        return {"success": True, "tx": str(tx_hash)}
